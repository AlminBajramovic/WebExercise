...
Flight::route('POST /customers/add', function() {
    ...
    */
    ...
    if($payload['first_name'] == NULL || $payload['first_name'] == '') {
        Flight::halt(500, "Customer name field is missing");
    }
    
    $customer = Flight::examService()->add_customer($payload);
    Flight::json(['message' => "Customer successfully added", 'data' => $customer]);
});
...
Flight::route('GET /foods/report', function(){
    ...
    */
    ...
    // Get all foods from service
    $allFoods = Flight::examService()->foods_report();
    
    // Calculate pagination
    $total = count($allFoods);
    $offset = ($page - 1) * $limit;
    $paginatedFoods = array_slice($allFoods, $offset, $limit);
